...
ark_code = addr_data_file.replace('addr-', '').replace('.json', '')
...
if res_len > 1:
    logging.warning('file={} len of suggestions={}'.format(addr_data_file, len(addr_info['suggestions'])))
elif 1 == res_len:
    ...
    print('addr_city={0}, addr_street={1}, addr_house={2}, addr_block={3}, addr_flat={4}'.format(ret_addr_city, ret_addr_street, ret_addr_house, ret_addr_block, ret_addr_flat) )
    ...
    addr_block = addr_info['suggestions'][0]['data']['block']  # корпус/строение
    addr_flat = addr_info['suggestions'][0]['data']['flat']  # квартира/офис
    logging.debug('addr_city={}, addr_kladr_street={}, addr_house={}, addr_block={}, addr_flat={}'.format(addr_city, addr_kladr_street, addr_house, addr_block, addr_flat) )
    print('{}^{}^{}^{}^{}'.format(ark_code, addr_kladr_street, addr_house, addr_block, addr_flat))
    """
else:
